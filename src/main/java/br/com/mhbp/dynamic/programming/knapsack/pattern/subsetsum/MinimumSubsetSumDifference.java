...
public class MinimumSubsetSumDifference {
    ...
    */
    ...
    public int canPartitionBruteForce(int[] num) {
        return this.canPartitionRecursiveBruteForce(num, 0, 0, 0);
    }

    private int canPartitionRecursiveBruteForce(int[] num, int currentIndex, int sum1, int sum2) {
        // base check
        if (currentIndex == num.length)
            return Math.abs(sum1 - sum2);

        // recursive call after including the number at the currentIndex in the first set
        int diff1 = canPartitionRecursiveBruteForce(num, currentIndex+1, sum1+num[currentIndex], sum2);

        // recursive call after including the number at the currentIndex in the second set
        int diff2 = canPartitionRecursiveBruteForce(num, currentIndex+1, sum1, sum2+num[currentIndex]);
